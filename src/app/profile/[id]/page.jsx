...
export default function ProfileDetailPage() {
  ...
  if (pageLoading) {
    return <div className="profile-detail profile-detail--not-found"><p>Loading...</p></div>;
  }
    ...
    <>
      <div className="profile-detail">
        <div className="profile-detail__inner">
          ...
          <button className="profile-detail__back" onClick={() => router.back()}>
            ← Back to Browse
          </button>
          ...
          <div className="profile-detail__grid">
            ...
            <div className="profile-detail__photo-col">
              <div className={`profile-detail__photo ${(!isAccepted && !isOwnProfile && profile.blur_photo) ? "profile-detail__photo--blurred" : ""}`}>
                {profile.image_url ? (
            ...
            <div className="profile-detail__info-col">
              ...
              <p className="profile-detail__tagline">{profile.profession} • {profile.city}</p>

              {(!isAccepted && !isOwnProfile) ? (
                <div className="profile-detail__locked">
                  <div className="profile-detail__locked-icon">🔒</div>
                  <h3 className="profile-detail__locked-title">Profile Details are Private</h3>
                ...
                <>
                  <div className="profile-detail__about">
                    ...
                    <p className="profile-detail__about-text">{profile.about || "No description added yet."}</p>
                  </div>

                  <div className="profile-detail__details">
                    <h3 className="profile-detail__section-title">Profile Details</h3>
                    <div className="profile-detail__table">
                      {details.map((d) => (
                        <div key={d.label} className="profile-detail__row">
                          <span className="profile-detail__row-label">{d.label}</span>
                          <span className="profile-detail__row-value">{d.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
