...
function BrowseContent() {
  ...
  const filtered = allProfiles.filter((p) => {
    ...
    const statusMatch = statusFilter === "All" || p.maritalStatus === statusFilter;
    ...
    <div className="browse-page">
      <div className="browse-page__inner">
        ...
        <aside className="browse-page__sidebar">
          ...
          <div className="browse-page__filter-group">
            <label className="browse-page__filter-label">Age: {ageMin} – {ageMax}</label>
            <div className="browse-page__age-row">
              <input
                type="number" min={18} max={ageMax} value={ageMin}
                onChange={(e) => setAgeMin(Number(e.target.value))}
                className="browse-page__age-input" placeholder="Min"
              />
              <span>to</span>
              <input
                type="number" min={ageMin} max={80} value={ageMax}
                onChange={(e) => setAgeMax(Number(e.target.value))}
                className="browse-page__age-input" placeholder="Max"
              />
            </div>
          </div>
        </aside>
            ...
            {filtered.length > 0 ? filtered.map((p, index) => (
              ...
              >
                ...
                <div className="browse-card__body">
                  ...
                  <p className="browse-card__meta">{p.age} yrs • {p.city}</p>
              ...
              <p className="browse-page__empty">No profiles match your filters.</p>
            )}
          </div>
        </main>
...
// main export wraps BrowseContent in Suspense
export default function BrowsePage() {
  return (
    <>
      <Suspense fallback={<div style={{ padding: "100px 24px", minHeight: "100vh" }}>Loading...</div>}>
        <BrowseContent />
      </Suspense>
      <CTABanner />
      <Footer />
